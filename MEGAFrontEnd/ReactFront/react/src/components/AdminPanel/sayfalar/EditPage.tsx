...
// 🟢 EKLENDİ: Rapor kategorileri için yeni state
    ...
    const onPicked = async (field: string, e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const url = toPublicPath(fileName);
    ...
    const fetchData = useCallback(
        async (recordId: string) => {
            ...
            try {
                ...
                // HABERLER
                if (isHaberMode) {
                    const data = await getHaberById(numericId);
                    ...
                    setFormData({
                        ...
                        haberTarih: (data.haberTarih ?? "").slice(0, 10),
                ...
                if (isYayinMode) {
                    ...
                    let data: any;
                    try { data = await apiGet<any>(`${cfg.apiEndpoint}/${numericId}`); }
                    catch {
                        try { data = await apiGet<any>(`${cfg.apiEndpoint}/find/${numericId}`); }
                        catch {
                            const all = await apiGet<any[]>(`${cfg.apiEndpoint}/list`).catch(() => apiGet<any[]>(cfg.apiEndpoint));
                ...
                if (isRaporMode) {
                    let data: any;
                    try { data = await apiGet<any>(`${RAPORLAR_CONFIG.apiEndpoint}/find/${numericId}`); }
                    catch {
                        try {
                    ...
                    setFormData({
                        ...
                        raporTarihi: (pick(data, "raporTarihi", "RAPOR_TARIHI") || "").toString().slice(0, 10),
                    });
                    setHasLoaded(true);
                    return;
                }
                ...
                if (isEventMode) {
                    let data: any;
                    try { data = await apiGet<any>(`/api/etkinlikler/${numericId}`); }
                    catch {
                ...
                if (isHizmetMode) {
                    let data: any;
                    try { data = await apiGet<any>(`${HIZMETLER_CONFIG.apiEndpoint}/${numericId}`); }
                    catch {
                ...
                if (isYonetimMode) {
                    ...
                    const initial: Record<string, any> = { id: extractId(data) };
                ...
                if (isKurumsalBMVIMode) {
                    ...
                    for (const kategori of categories) {
                        try {
                            ...
                            if (d) { foundData = d; category = kategori; break; }
                        } catch {}
                    }
                    ...
                    config.fields.forEach((f) => {
                        if (initial[f.name] === undefined) initial[f.name] = pick(foundData, f.name);
                    });
    ...
    useEffect(() => { if (id && !hasLoaded) fetchData(id); }, [id, hasLoaded, fetchData]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isYayinMode) {
                ...
                try { await apiPut(`/api/yayinlar/${idForPut}`, payload); }
                catch { await apiPut(`/api/yayinlar/update/${idForPut}`, payload); }
            ...
            if (isHaberMode) {
                ...
                await updateHaber(formData.haberlerId, payload);
                alert("Haber güncellendi!");
                navigate(goBackToList(), { replace: true });
                return;
            }
            ...
            if (isYonetimMode) {
                ...
                const base = TABLE_CONFIGS["kurumsal_yonetim_semasi"].apiEndpoint;
                try { await apiPut(`${base}/${idForPut}`, payload); }
                catch { await apiPut(`${base}/update/${idForPut}`, payload); }
                alert("Yönetim şeması güncellendi!");
                return;
            }

            if (isKurumsalBMVIMode && tableConfig?.tableName === "KURUMSAL_BASKAN_MISYON_VIZYON_ILKELERIMIZ") {
                const updateData = {
    ...
    // 🔧 Resim alanları için: metin + Dosya Seç + gizli file input + önizleme
        ...
        return (
            <>
                <div className="flex items-center gap-2">
                    {/* ARTIK SADECE OKUNUR: el ile yazılamaz */}
                    <input
